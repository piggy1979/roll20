...
function addAbility(str, charID, isRanged) {
    ...
    for (var i = 0; i < attackPlus.length; i++) {
        ...
        createObj("ability", {
            name: abilityName,
            description: "",
            action: gEMString + " " + abilityName + " Attacks = [[1d20 + " + attackPlus[i] +"]]"+ critStr + abilityLink,
            istokenaction: true,
            characterid: charID
        });
    }
    ...
    else
    {
        ...
        createObj("ability", {
            name: damageName,
            description: "",
            action: gEMString+" " + attackName + " Damage = [[" + dmgStr  + "]] " + dmgDescr,
            istokenaction: true,
            characterid: charID
        });
    }
...
log("Start creating char")        
        ...
        var attribList = new Array("Init", "Perception", "AC", "touch", "flat-footed", "hp", "Fort", "Ref", "Will", "Speed");
        ...
        var ac = findObjs({ _type: "attribute", name: "AC", _characterid: charID})[0];    
        var speed = findObjs({ _type: "attribute", name: "Speed", _characterid: charID})[0]; 
        
        token.set("name", mobName);
        token.set("showname", true);
        token.set("bar1_value", hp.get("current"));
        token.set("bar1_max", hp.get("max"));
        token.set("bar2_value", speed.get("current"));
        token.set("bar3_value", ac.get("current"));
